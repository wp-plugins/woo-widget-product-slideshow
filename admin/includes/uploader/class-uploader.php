...
-----------------------------------------------------------------------------------*/
class WC_Product_Slider_Uploader extends WC_Product_Slider_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'wc_product_slider' ).'" rel="'.$post_id.'" /> '.$description;
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'wc_product_slider' );
	...
	public function change_button_text( $translation, $original, $domain ) {
	    ...
	    if ( is_admin() && $original === 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'wc_product_slider' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'wc_product_slider' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'wc_product_slider' ), $tabs['gallery'] ); }
		return $tabs;
	}
...
global $wc_product_slider_uploader;
$wc_product_slider_uploader = new WC_Product_Slider_Uploader();
